 ...
 * The State Machine
 */
final class BasinFrameSM extends ComponentStateMachine {

    private final BasinSMItf frame;

    BasinFrameSM(final BasinSMItf frame) {
        super("BasinSM", BasinState.init);
    ...
    private final Condition renderStart = new Condition() {
        public boolean condition(final Input i) {
            if (!((BasinManager) frame.getManager()).doRendering()) {
